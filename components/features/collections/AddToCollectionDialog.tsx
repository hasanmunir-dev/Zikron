...
export function AddToCollectionDialog({ open, onOpenChange, itemType, itemId }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-card flex flex-col shadow-xl border border-border rounded-2xl w-full max-w-sm max-h-[90vh]"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border shrink-0">
          <h2 className="text-base font-semibold text-foreground">Add to Collection</h2>
          <button type="button" onClick={handleClose} aria-label="Close" className="p-1.5 text-muted-foreground hover:text-foreground transition-colors">
            <X size={18} />
          </button>
        </div>
